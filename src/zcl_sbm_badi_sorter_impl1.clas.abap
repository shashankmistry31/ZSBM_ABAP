class ZCL_SBM_BADI_SORTER_IMPL1 definition
  public
  final
  create public .
...
CLASS ZCL_SBM_BADI_SORTER_IMPL1 IMPLEMENTATION.


METHOD IF_BADI_SORTER~GET_DATA_FROM_SCREEN.
  DATA layer TYPE i.

  CALL FUNCTION 'ZSBM_BADI_SORTER_LAYER_GET'
    IMPORTING
      layer = layer.
...
METHOD IF_BADI_SORTER~PUT_DATA_TO_SCREEN.
  ...
  CALL FUNCTION 'ZSBM_BADI_SORTER_LAYER_PUT'
    EXPORTING
      layer = layer.
...
method IF_BADI_SORTER~SET_LAYER_CHANGEABLE.
  CALL FUNCTION 'ZSBM_SORTER_LAYER_SET_CHANGE'
    EXPORTING
